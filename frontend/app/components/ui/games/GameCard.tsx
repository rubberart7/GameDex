...
export interface Game {
    ...
    name: string;
    rating: number;
    platforms?: (PlatformObj | Platform)[];
    parent_platforms?: (PlatformObj | Platform)[];
    released: string;
...
const GameCard: React.FC<GameCardProps> = ({ game }) => {
    ...
    return (
        <Link href={`/game-details/${game.id}`} passHref>
            <div className="relative bg-slate-950 rounded-lg shadow-md overflow-hidden w-full min-h-[475px] transition-transform duration-300 hover:scale-108 cursor-pointer">
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <div className="flex flex-wrap gap-1">
                            {rawPlatforms.length > 0 ? (
                                rawPlatforms.map((item: PlatformObj | Platform, idx: number) => {
                                    const platform = 'platform' in item ? item.platform : item;
                                    return (
                                        <span
                                            key={`platform-${platform.id || platform.slug || idx}`}
